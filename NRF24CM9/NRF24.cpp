 ...
 *
 *********************************************************/
bool NRF24::begin(uint8 cePin, uint8 csnPin, uint32 _netmask)
{
...
void NRF24::setAddress(uint8 address)
{
	ownAddress = address;
...
int8 NRF24::listenToAddress(uint8 address)
{
	if (numPipes >= 5) return -1;

	uint8 pipeIndex = numPipes + 1;
...
void NRF24::setChannel(uint8 channel)
{
...
{
	uint8 rfSetup = readRegister(RF_SETUP);
...
bool NRF24::broadcast(uint8 *data, uint8 length)
{
...
{
	return broadcast((uint8 *)message, sizeof(message) + 1);
}

/********************************************************/

// bool NRF24::broadcast_P(const char FlashStringHelper *message)
// {
...
bool NRF24::send(uint8 targetAddress, uint8 *data, uint8 length, uint8 *numAttempts)
{
	...
	if (numAttempts)
	{
		...
		uint8 observe = readRegister(OBSERVE_TX);
...
int8 NRF24::send(uint8 targetAddress, uint8 *data, uint8 length, uint8 *responseBuffer, uint8 bufferSize, uint8 *numAttempts)
{
...
bool NRF24::send(uint8 targetAddress, char *message)
{
	// Both TX and RX on pipe 0 must match the target address
	// RX is required to receive ACK

	return send(targetAddress, (uint8 *)message, sizeof(message) + 1, NULL);
}

/********************************************************/

bool NRF24::queueResponse(uint8 *data, uint8 length)
{
...
uint8 NRF24::available(uint8 *listener)
{
	uint8 status = readRegister(STATUS);
...
uint8 NRF24::read(uint8 *buf, uint8 bufferSize)
{
	// disable RX mode
	ceLow();

	uint8 payloadSize = readRegister(R_RX_PL_WID);
...
uint8 NRF24::read(char *buf, uint8 bufferSize)
{
	uint8 bytesRead = read((uint8 *)buf, bufferSize - 1);
	buf[bufferSize - 1] = '\0';
	return bytesRead;
}
...
{
	uint8 config = readRegister(CONFIG);
	config &= ~PWR_UP;
...
nrf24_mode_e NRF24::getCurrentMode()
{
	...
	uint8 fifoEmpty = readRegister(FIFO_STATUS) & TX_EMPTY;
...
void NRF24::setRetries(uint8 delay, uint8 count)
{
...
uint8 NRF24::readRegister(uint8 reg)
{
	csnLow();
	SPI.write(R_REGISTER | reg);
	uint8 result = SPI.read();
	csnHigh();
...
void NRF24::writeRegister(uint8 reg, uint8 value)
{
	csnLow();
	SPI.write(W_REGISTER | (REGISTER_MASK & reg));
	SPI.write(value);
	csnHigh();
}
...
void NRF24::writeRegister(uint8 reg, uint8 *value, uint8 numBytes)
{
	csnLow();
	SPI.write(W_REGISTER | (REGISTER_MASK & reg));
	while (numBytes--)
...
bool NRF24::transmit(uint8 targetAddress, uint8 *data, uint8 length, bool ack)
{
	...
	static const uint16 timeout = 500;
	uint32 txStarted = millis();
...
void NRF24::assembleFullAddress(uint8 address, uint8 buf[5])
{
